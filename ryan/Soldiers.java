...
public class Soldiers extends RobotPlayer {
    ...
    public static FastLocSet invalidSrpLocs = new FastLocSet();
    ...
    public static void paintUnder() throws GameActionException {
        ...
        if (rc.isActionReady() && rc.getPaint() > 30) {
     ...
     *   1. Do we have enough chips to build a tower?
     *   2. If there are no ruins, find a ruin without enemy paint. Set this as the new current ruin.
     *   3. Check if the current ruin is still valid (no enemy paint, no towers built on it)
     * Notes: 
    ...
    public static boolean shouldCompleteTower() throws GameActionException {
        if (nearbyRuins.length == 0 || rc.getChips() < 1000) {
    ...
    public static boolean shouldCompleteSrp() throws GameActionException {
        if (rc.getChips() < 200) return false;
        // If no target Srp, find the nearest possible center of Srp
        if (currentSrpLoc == null) {
            for (MapInfo tile : nearbyTiles) {
                if (tile.getMark() == PaintType.ALLY_PRIMARY && !invalidSrpLocs.contains(tile.getMapLocation())) {
        ...
        if (currentSrpLoc != null) {
            ...
            if (nearestWrongInSrp != null && nearestWrongInSrp.equals(new MapLocation(-1, -1))) {
                invalidSrpLocs.add(currentSrpLoc);
