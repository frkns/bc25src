...
public class CompleteSrp extends Action {
        ...
        if (_Info.srpCenter != null) {
            ...
            if (rc.canCompleteResourcePattern(_Info.srpCenter)) {
                ...
                if (validatePotentialCenters()){
                    ...
                    MapLocation corner1 = _Info.srpCenter.translate(-2, -2);
                    MapLocation corner2 = _Info.srpCenter.translate(-2, 2);
                    MapLocation corner3 = _Info.srpCenter.translate(2, -2);
                    MapLocation corner4 = _Info.srpCenter.translate(2, 2);
                    
                    if (!_Info.srpMarkedCorners.contains(corner1)) {
                        _Info.tilesToMark.add(corner1);
                    }
                    if (!_Info.srpMarkedCorners.contains(corner2)) {
                        _Info.tilesToMark.add(corner2);
                    }
                    if (!_Info.srpMarkedCorners.contains(corner3)) {
                        _Info.tilesToMark.add(corner3);
                    }
                    if (!_Info.srpMarkedCorners.contains(corner4)) {
                        _Info.tilesToMark.add(corner4);
                    }
                    if (cursor == null) {
                        spawnCursor();
                    }
                    ...
                    if (completed == true){
                        Debug.print(3, "[CompleteSrp] Pattern completed", debugAction);
                        if (!_Info.processedBlockerTiles.contains(_Info.srpCenter)) _Info.blockerTiles.add(_Info.srpCenter);
                        _Info.srpMarkedCorners.clear();
                        markInvalid();
                        score = 0;
                     ...
                     // If we have reached the end of pattern, we reset the cursor at the beginning
                } else {
     ...
     * then shiftsf horizontally and reverses vertical direction when reaching the band limits.
     * 
     * Pattern example:
     * ↓→↑→↓→↑
     */
    ...
    public boolean validatePotentialCenters() throws GameActionException {
        ...
        for (MapInfo tile : srpTiles) {
            if (tile.getMark() == PaintType.ALLY_PRIMARY && tile.getMapLocation().isWithinDistanceSquared(_Info.srpCenter, 7)) {
                Debug.print(3, "[CompleteSrp] Found SRP corner at: " + tile.getMapLocation(), debugAction);
                if (!_Info.processedCorners.contains(tile.getMapLocation())) _Info.corners.add(tile.getMapLocation());
                return false;
            }
            if (tile.isWall() ||
