...
public class FindSrpCenter extends Interest {
    ...
    public void updateDirectionScores() throws GameActionException {
        ...
        } else { // Give the action a chance to validate before moving there, since for now we are only looking a the 3x3 area around the robot.
            adjustDirectionScore(rc.getLocation().directionTo(_Info.srpCenter).rotateLeft(), Constants.FindSrpCenterScoreRotate);
            adjustDirectionScore(rc.getLocation().directionTo(_Info.srpCenter), Constants.FindSrpCenterScore);
            adjustDirectionScore(rc.getLocation().directionTo(_Info.srpCenter).rotateRight(), Constants.FindSrpCenterScoreRotate);
