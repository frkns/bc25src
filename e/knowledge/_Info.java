...
public class _Info {
    ...
    public static FastLocSet knownSrpCenters = new FastLocSet(); // SRP centers that have been completed or in progress
    ...
    public static void processBlockerTiles() {
        MapLocation blockerTile; // 500 bytecode per blocker tile
        while ((blockerTile = blockerTiles.pop()) != null && Clock.getBytecodesLeft() > 1000) {            
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x - 2) + (char)(blockerTile.y - 2));
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x - 2) + (char)(blockerTile.y - 1));
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x - 2) + (char)(blockerTile.y));
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x - 2) + (char)(blockerTile.y + 1));
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x - 2) + (char)(blockerTile.y + 2));
            
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x - 1) + (char)(blockerTile.y - 2));
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x - 1) + (char)(blockerTile.y - 1));
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x - 1) + (char)(blockerTile.y));
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x - 1) + (char)(blockerTile.y + 1));
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x - 1) + (char)(blockerTile.y + 2));
            
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x) + (char)(blockerTile.y - 2));
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x) + (char)(blockerTile.y - 1));
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x) + (char)(blockerTile.y));
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x) + (char)(blockerTile.y + 1));
            invalidSrpCenters.addAsString("^" + (char)(blockerTile.x) + (char)(blockerTile.y + 2));
