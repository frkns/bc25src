...
public class UtilsSrp extends architecture.RobotPlayer{
        // ---- Action variables ----
        ...
        static char MIN_SCORE_FOR_MARK = (char) ((int) ZERO + 10); // Mark if score >= ZERO
        ...
        public static void init() {
            ...
            history = "\u0000".repeat(70 * 70).toCharArray(); // Default to empty paint
        }
        ...
        public static boolean updateScores() throws GameActionException {
            architecture.Tools.Debug.println("\t\tUpdating scores.");
            for (MapInfo info : rc.senseNearbyMapInfos()) {

                if (Clock.getBytecodesLeft() < 9000) {
                    architecture.Tools.Debug.println("\tW - Update SRP score : Need more time to update. Skipping to next action.");
                    return false;
                }
            ...
            architecture.Tools.Debug.println("\t\tDone !");
            // printScores();
            return true;
        }
        ...
        public static void run() throws GameActionException {
            ...
            if (targetMark == null) {
                architecture.Tools.Debug.println("\tX - ACTION_MARK_SRP      : No target to mark");
                action = Action.ACTION_WAITING_FOR_ACTION;
                return;
            }

            architecture.Tools.Debug.println("Score min : " + (int) MIN_SCORE_FOR_MARK);
            architecture.Tools.Debug.println("Best cell " + targetMark + " (score " + (int) scoreTargetMark + " | " + ((int) scoreTargetMark - (int) ZERO) + ")");

            //------------------------------------------------------------------------------//
            // Play Action
            //------------------------------------------------------------------------------//
            architecture.Tools.Debug.println("\t0 - ACTION_MARK_SRP      : Playing!");
            ...
            if (rc.canMark(targetMark) && rc.getLocation().isWithinDistanceSquared(targetMark, 16)) {
                architecture.Tools.Debug.println("\t\tMarking cell " + targetMark + " (id" + (SHIFT_START + targetMark.x + 70 * targetMark.y) + ")");
        ...
        public static void printScores() throws GameActionException {
            ...
            architecture.Tools.Debug.println("Scores :");
